...
class StaticTestCase(TestCaseMixin, unittest.TestCase):
    db_prefix = "StaticTestCase"

    def test_no_exception(self):
        db_file = "test_no_exception"
        app, db, error_manager = self.setUpApp(db_file)
        with app.test_client() as c:
            result = c.get('/')
            self.assertEqual(u'No Exception!', result.data.decode('utf-8'))
            self.assertEqual(error_manager.get_all_errors(), [])
        self.tearDownApp(db)
    ...
    def test_value_error(self):
        ...
        with app.test_client() as c:
            ...
            self.assertEqual(error_new.exception, error.exception)
